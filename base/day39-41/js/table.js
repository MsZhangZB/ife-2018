    ...
    for (var j = 0; j < sale_len; j++) {
      ...
      sales.onclick = function (event) {
        ...
        if (target.nodeName === 'TD') {
          input = target.querySelector('input[type=text]');
          span = target.querySelector('span');
          btn = target.querySelectorAll('button');  
        }
        if (target.nodeName === 'SPAN') {
          input = target.parentNode.querySelector('input[type=text]');
          span = target.parentNode.querySelector('span');
          btn = target.parentNode.querySelectorAll('button'); 
        }
